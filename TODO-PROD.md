...
- **Branches remotas:** inventariadas todas via `git ls-remote --heads origin`; as 25 branches de PRs fechados foram removidas (via API do GitHub, sem afetar branches com PR aberto). Restam apenas `main`, `codex/060-read-only-production-smoke` (PR #400, aberto) e `codex/064-reconcile-associate-identities` (PR #399, aberto).
...
- **Pendência remanescente:** até o Plano 060 (contenção do smoke de produção) ser aplicado, este inventário deve ser repetido após cada execução mutante do smoke.
