...
| Smoke produção        | produção live                                       | reais; `SMOKE_<run-id>_*` somente em dispatch mutante | push pós-deploy read-only vinculado ao SHA; mutações só em janela manual com conta dedicada | sem migration; limpeza manual run-scoped após mutações e preservação integral de `audit_logs`                                                                                     |
