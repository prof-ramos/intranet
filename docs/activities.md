...
**Status:** Fase 0 / preparação para F3 (timeline consolidada)
...
- `src/lib/activities/service.ts`: `createActivityService` e `updateActivityService`. A criação e a atualização fazem mutação + outbox na mesma `db.transaction()`, usam bloqueio otimista por `xmin` e registram auditoria pós-commit com `logAuditBestEffort`.
- `src/lib/activities/repository.ts`: `findActivities` (listagem deliberadamente limitada), `findActivityById`, `insertActivity`, `updateActivityById` e `listActivityTimeline`. A timeline atual lê `audit_logs` filtrando `entityType = 'activity'`.
...
- `src/app/app/atividades/actions.ts`: `createActivity`, `createQuickActivityAction`, `updateActivityAction` e `getActivityTimelineAction`, todas autorizadas para `admin`, `diretoria` e `secretaria`. `describeTimelineEntry` ainda conhece apenas criação e atualização.
- `src/app/app/atividades/AtividadesBoard.tsx` e `src/app/app/atividades/_board/Drawer.tsx`: board, atualização otimista, drawer, dados e histórico; não há seção de comentários nem labels estruturadas.
- `src/lib/integrations/outbox.ts` e `src/lib/db/schema/integrations.ts`: outbox de webhooks, com `domain_events`, `actorAdminId`, retenção e dispatch separado. Os tipos `activity.*` existentes são os relacionados na tabela de eventos abaixo.
...
- `drizzle/postgres/`: migrations numeradas até `0036_encrypt_secondary_email.sql`. A migration `0028_activity_domain_events.sql` registra o padrão atual de expansão dos enums do outbox.
...
| Timeline | `listActivityTimeline` lê `audit_logs`; `describeTimelineEntry` cobre criação/alteração | Normalizar Quem/O quê/Quando e estender a descrição para comentários e labels; avaliar leitura complementar de `domain_events` sem duplicar armazenamento | `src/lib/activities/repository.ts`, `src/app/app/atividades/actions.ts` |
| Eventos | Seis eventos de atividade no outbox; dois gatilhos in-app | Acrescentar os tipos faltantes de RF-04 e preservar granularidade, atomicidade e idempotência do outbox | `src/lib/activities/domain-events.ts`, `src/lib/integrations/outbox.ts` |
...
| Migrations | 36 migrations numeradas até 0036 | Somente migrations aditivas, compatíveis com deploy gradual e rollback lógico | `drizzle/postgres/`, `drizzle/postgres/0028_activity_domain_events.sql` |
...
| **F1 — Comentários** | Comentários institucionais no drawer e na API | Não há tabela, repository, service ou action de comentário | `activity_comments` com `deleted_at`, repository/service/actions, validação e sanitização, auditoria, eventos `activity.comment_*`, UI e testes; `ACTIVITY_COMMENTS_ENABLED` |
| **F2 — Labels** | Labels administráveis e filtro | Só há `activities.tags` JSONB, com área/tags livres normalizadas; não há labels estruturadas | `activity_labels` e `activity_label_assignments`, seed configurável, CRUD administrativo mínimo, associação/remoção, eventos, auditoria, UI e `label` na URL; `ACTIVITY_LABELS_ENABLED` |
| **F3 — Timeline consolidada** | Histórico que responda Quem? O quê? Quando? | `listActivityTimeline` consulta `audit_logs`; `describeTimelineEntry` só descreve criação/atualização | Normalização de valores anterior/novo; descrição de comentários/labels; eventual leitura complementar de `audit_logs` + `domain_events`, mantendo `audit_logs` como fonte da timeline da UI; testes |
| **F4 — UX do board** | Tornar o board uma superfície institucional | Card/drawer já existem, mas sem labels, comentários e seus indicadores | Card compacto com responsável, prazo, prioridade, labels, comentários e atraso; drawer consolidado; edição inline seletiva; loading, erro, acessibilidade e mobile |
...
| `activity.label_added` | **Não** | Depende de `activity_labels`/associações da F2 |
| `activity.label_removed` | **Não** | Depende de `activity_labels`/associações da F2 |
| `activity.comment_added` | **Não** | Não há `activity_comments`; adicionar com a F1 |
| `activity.comment_edited` | **Não** | Não há comentário nem evento correspondente; adicionar com a F1 |
| `activity.comment_deleted` | **Não** | A exclusão prevista é lógica; adicionar evento com a F1 |
