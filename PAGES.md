...
### `/app/config/mcp` — Tokens MCP

**Acesso:** `admin`, `diretoria`, `secretaria`

**Funcionalidades:**

- Criação do próprio PAT de operador, com validade obrigatória de 90 dias
- Checkbox obrigatório de ciência LGPD sobre o uso do provedor LLM como subprocessador
- Exibição do token `asof_mcp_...` uma única vez após a criação
- Listagem e revogação dos próprios tokens
- Revogação de qualquer token por `admin`

**Funcional quando:**

- [ ] Token criado sem ciência LGPD é rejeitado
- [ ] PAT plaintext não pode ser recuperado após fechar a exibição inicial
- [ ] Operador pode revogar o próprio token
- [ ] `admin` pode revogar token de qualquer operador
- [ ] Token revogado ou expirado retorna 401 em `/api/mcp`
