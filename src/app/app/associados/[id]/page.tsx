...
type ProfileSectionProps = {
  profile: NonNullable<Awaited<ReturnType<typeof getAssociateProfile>>>;
  id: string;
};

function OverviewSection({ profile, id }: ProfileSectionProps) {
...
function InactiveWarningSection({ profile }: Omit<ProfileSectionProps, 'id'>) {
  const { isAssociationActive } = profile;
  if (isAssociationActive) return null;
  return (
  ...
  return (
    <SectionCard id="dados-profissionais" title="Dados Profissionais">
      <dl className="m-0">
        ...
        <Row label="Data de aposentadoria" value={formatAssociateDate(associate.retirementDate)} />
        <Row label="Data de licença" value={formatAssociateDate(associate.leaveDate)} />
        <Row
  ...
  return (
    ...
    >
      <dl className="m-0">
        ...
        <Row label="Data de adesão à ASOF" value={formatAssociateDate(associate.joinedAt)} />
        <Row label="Vínculo ASOF" value={getAssociateStatusLabel(associate.associationStatus)} />
...
function PaymentHistorySection({ profile }: Omit<ProfileSectionProps, 'id'>) {
...
function ActivitiesSection({ profile }: Omit<ProfileSectionProps, 'id'>) {
    ...
    <main className="mx-auto w-full max-w-[1180px] min-w-0 px-5 py-7 sm:px-8 lg:px-10">
      ...
      <div className="grid items-start gap-7 xl:grid-cols-[200px_minmax(0,1fr)]">
        ...
        <div className="flex min-w-0 flex-col gap-7">
          <OverviewSection profile={profile} id={id} />
          <InactiveWarningSection profile={profile} />
          <IdentificationSection profile={profile} id={id} />
          <AddressSection profile={profile} id={id} />
          <ProfessionalDataSection profile={profile} />
