    ...
    <div className="mx-auto w-full max-w-[1180px] px-5 py-7 sm:px-8 lg:px-10">
      ...
      <div className="mb-6 flex flex-wrap gap-3">
        {Array.from({ length: 4 }).map((_, i) => (
          <div key={i} className="h-10 w-28 animate-pulse rounded-lg bg-base-200" />
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
        {Array.from({ length: 4 }).map((_, col) => (
          <div key={col} className="rounded-box min-h-[400px] border border-base-200 bg-base-100 p-3">
            {/* Column header */}
            <div className="mb-3 flex items-center justify-between">
              <div className="flex items-center gap-2">
                <div className="h-2.5 w-2.5 animate-pulse rounded bg-base-300" />
            ...
            <div className="flex flex-col gap-2">
              {Array.from({ length: 3 }).map((_, card) => (
                <div
                  key={card}
                  className="rounded-[8px] border border-base-200 bg-white p-3"
                >
                  <div className="mb-2 h-4 w-full animate-pulse rounded bg-base-200" />
                  <div className="mb-1 h-3 w-16 animate-pulse rounded bg-base-200" />
                  <div className="flex items-center justify-between">
                    <div className="h-3 w-20 animate-pulse rounded bg-base-200" />
                    <div className="h-3 w-14 animate-pulse rounded bg-base-200" />
                  </div>
                </div>
              ))}
            </div>
