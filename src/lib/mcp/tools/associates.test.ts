...
vi.mock('@/lib/search/queries', () => ({
  searchAssociates: mocks.searchAssociates,
...
describe('handlers MCP do Cadastro de Oficiais', () => {
  ...
  it('descriptografa PII somente quando solicitada e audita o acesso', async () => {
    ...
    const result = await getAssociate({ id: 1, includeSensitive: true }, principal);
  ...
  it('rejeita acesso a dados sensíveis para papel não autorizado', async () => {
    ...
    const result = await getAssociate({ id: 1, includeSensitive: true }, unauthorizedPrincipal);
  ...
  it('busca global consulta oficiais e atividades sem aceitar CPF/SIAPE', async () => {
    mocks.searchAssociates.mockResolvedValue([
      { type: 'associate', id: 1, title: 'Ana', subtitle: 'SERE', href: '/app/associados/1' },
    ]);
    mocks.searchActivities.mockResolvedValue([
      {
        ...
        subtitle: 'A fazer',
        href: '/app/atividades?open=9',
      },
    ]);

    const result = await globalSearch({ query: 'Ana' }, principal);

    expect(mocks.searchAssociates).toHaveBeenCalledWith('Ana', 5);
    expect(mocks.searchActivities).toHaveBeenCalledWith('Ana', 5);
    expect(result.structuredContent).toEqual({
      associates: [
        { type: 'associate', id: 1, title: 'Ana', subtitle: 'SERE', href: '/app/associados/1' },
      ],
  ...
  it('lista dependentes e convênios após validar o oficial com DTOs seguros', async () => {
    ...
    const agreements = await listAssociateHealthAgreements(
      { associateId: 1, includeSensitive: true },
      principal,
    );
  ...
  it('omite convênios de saúde quando includeSensitive for false', async () => {
    mocks.findAssociateById.mockResolvedValue({ id: 1 });
    mocks.findHealthAgreementsByAssociateId.mockResolvedValue([
    ...
    const agreements = await listAssociateHealthAgreements(
      { associateId: 1, includeSensitive: false },
      principal,
    );
