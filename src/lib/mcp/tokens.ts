...
export async function rotateOperatorMcpToken({
  id,
  actorId,
}: {
  id: number;
  actorId: number;
}): Promise<(OperatorMcpTokenListItem & { token: string }) | null> {
  assertPositiveId(id, 'Token');
  assertPositiveId(actorId, 'Ator');
  ...
  return db.transaction(async (tx) => {
    ...
    await logAuditAction({
      ...
      entityType: 'admin',
      entityId: token.adminId,
      metadata: { channel: 'mcp', previousTokenId: id, tokenId: created.id },
      executor: tx,
    });
    ...
    return { ...created, adminName: owner?.name ?? '', token: rawValue };
  });
}
