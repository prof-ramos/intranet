...
export async function verifyMcpAuth(
  ...
  // O papel do administrador é carregado como a única "scope" do AuthInfo para
  // que `toolsForRole` filtre o catálogo de ferramentas por papel. O papel NÃO
  // vem do token: `verifyOperatorMcpToken` relê `admins.role` do banco a cada
  // requisição, então revogar/desativar o administrador vale imediatamente.
  return {
    token,
