...
describe('tokens MCP de operador', () => {
  ...
  it.each([
    ...
    await expect(revokeOperatorMcpToken({ id: 9, actorId })).resolves.toBe(expected);
    expect(update.update).toHaveBeenCalledTimes(expected ? 1 : 0);
  });

  it('renova o token ativo e revoga o anterior', async () => {
    let selectCount = 0;
    const txSelect = vi.fn(() => ({
      from: vi.fn(() => ({
        where: vi.fn(() => ({
    ...
    const returningUpdate = vi.fn().mockResolvedValue([{ id: 9 }]);
    const returningInsert = vi.fn().mockResolvedValue([
      {
        id: 15,
        adminId: 4,
        name: 'Claude',
        lgpdAcknowledgedAt: new Date('2026-09-21T12:00:00Z'),
        lastUsedAt: null,
        expiresAt: new Date('2026-12-20T12:00:00Z'),
        revokedAt: null,
    ...
    const tx = {
      select: txSelect,
      update: vi.fn(() => ({
        set: vi.fn(() => ({ where: vi.fn(() => ({ returning: returningUpdate })) })),
      })),
      insert: vi.fn(() => ({
    ...
    const result = await rotateOperatorMcpToken({ id: 9, actorId: 1 });

    expect(result?.token).toMatch(/^asof_mcp_/);
    expect(result?.id).toBe(15);
    expect(result?.adminName).toBe('Operador');
    expect(mocks.logAuditAction).toHaveBeenCalledWith(
      expect.objectContaining({
