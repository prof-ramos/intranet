...
export const changePasswordSchema = z.object({
  ...
  newPassword: z.string().min(12, 'A nova senha deve ter pelo menos 12 caracteres.'),
...
export const updateAssociateSchema = z.object({
  ...
  cpf: z.string().trim().nullable().refine(cpfValidator, 'CPF em formato inválido.'),
  siape: z.string().trim().nullable().refine((v) => !v || /^\d{6,10}$/.test(v.replace(/\D/g, '')), 'SIAPE em formato inválido.'),
  primaryEmail: z.string().trim().email('E-mail principal inválido.').nullable().or(z.literal('')),
  secondaryEmail: z.string().trim().email('E-mail alternativo inválido.').nullable().or(z.literal('')),
  phone: z.string().trim().nullable(),
  whatsapp: z.string().trim().nullable(),
  birthDate: z.string().trim().refine(isValidDateString, 'Data de nascimento inválida.').nullable().or(z.literal('')),
  address: z.string().trim().nullable(),
  locationCity: z.string().trim().nullable(),
  locationCountry: z.string().trim().nullable(),
  assignment: z.string().trim().nullable(),
  assignmentStartDate: z.string().trim().refine(isValidDateString, 'Data de lotação inválida.').nullable().or(z.literal('')),
  classPattern: z.string().trim().nullable(),
  associationCategory: z.string().trim().nullable(),
  functionalStatus: z.enum(validFunctionalStatuses).nullable(),
  associationStatus: z.enum(validAssociationStatuses).nullable(),
  contributionStatus: z.enum(validContributionStatuses).nullable(),
});
