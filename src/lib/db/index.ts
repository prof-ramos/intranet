...
const client = postgres(databaseUrl, {
  ...
  max: positiveInteger(process.env.DB_MAX_CONNECTIONS, 5),
  connect_timeout: positiveInteger(process.env.DB_CONNECT_TIMEOUT_SECONDS, 10),
  idle_timeout: positiveInteger(process.env.DB_IDLE_TIMEOUT_SECONDS, 20),
  ssl:
