...
// Mock env module to control TRUSTED_PROXY_COUNT
const mockEnv = vi.hoisted(() => ({ TRUSTED_PROXY_COUNT: undefined as string | undefined }));
...
describe('getTrustedProxyCount', () => {
  ...
  it('falls back to 1 for invalid values', () => {
    mockEnv.TRUSTED_PROXY_COUNT = 'abc';
    expect(getTrustedProxyCount()).toBe(1);
