...
export const countConsultationsByStatus = unstable_cache(
  async (status: (typeof legalConsultationStatus.enumValues)[number]): Promise<number> => {
    const rows = await db
      .select({ count: count() })
...
export async function getConsultationsPaginated(
  ...
  if (filters.status) {
    conditions.push(eq(legalConsultations.status, filters.status as typeof legalConsultationStatus.enumValues[number]));
  }
