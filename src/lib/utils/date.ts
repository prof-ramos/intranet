...
// ⚡ Bolt: Cache Intl.DateTimeFormat instances to avoid expensive object creation on every call
const shortDtf = new Intl.DateTimeFormat('pt-BR', { timeZone: 'UTC' });
const longDtf = new Intl.DateTimeFormat('pt-BR', {
  day: '2-digit',
  month: 'long',
  year: 'numeric',
  timeZone: 'UTC',
});
const dueDtf = new Intl.DateTimeFormat('pt-BR', {
  day: '2-digit',
  month: 'short',
  timeZone: 'UTC',
});
...
export function formatLongDate(value: string | Date | null | undefined): string | null {
  ...
  return longDtf.format(new Date(Date.UTC(parts.year, parts.month - 1, parts.day)));
}
...
export function formatDueDate(value: string | null | undefined): string | null {
  ...
  if (!date) return null;
  return dueDtf.format(date).replace('.', '');
}
