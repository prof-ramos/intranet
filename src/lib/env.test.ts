...
describe('envSchema', () => {
  ...
  test('rejeita producao com INITIAL_ADMIN_EMAIL placeholder example.invalid', () => {
    const result = envSchema.safeParse({
      DATABASE_URL: 'postgres://user:pass@localhost:5432/db',
      DATABASE_MIGRATION_URL: 'postgres://user:pass@localhost:5432/db',
      SKIP_AUTH: 'false',
    ...
    expect(result.success).toBe(false);
    if (!result.success) {
      const issue = result.error.issues.find((i) => i.path.includes('INITIAL_ADMIN_EMAIL'));
      expect(issue?.message).toBe(
  ...
  test('rejeita producao com INITIAL_ADMIN_PASSWORD placeholder', () => {
    const result = envSchema.safeParse({
      DATABASE_URL: 'postgres://user:pass@localhost:5432/db',
      DATABASE_MIGRATION_URL: 'postgres://user:pass@localhost:5432/db',
      SKIP_AUTH: 'false',
    ...
    expect(result.success).toBe(false);
    if (!result.success) {
      const issue = result.error.issues.find((i) => i.path.includes('INITIAL_ADMIN_PASSWORD'));
      expect(issue?.message).toBe(
  ...
  test('aceita placeholders de admin em desenvolvimento', () => {
    const result = envSchema.safeParse({
      DATABASE_URL: 'postgres://user:pass@localhost:5432/db',
      SKIP_AUTH: 'false',
      SESSION_SECRET: 'test-session-secret-with-at-least-32-chars',
  ...
  test('aceita ASSINAFY_API_KEY com URL https de producao', () => {
    const result = envSchema.safeParse({
      ...validEnv,
      ASSINAFY_API_KEY: 'assinafy-api-key',
  ...
  test('aceita Assinafy desabilitado sem chave e sem URL', () => {
    const result = envSchema.safeParse(validEnv);

    expect(result.success).toBe(true);
    if (result.success) {
      expect(result.data.ASSINAFY_API_KEY).toBeUndefined();
      expect(result.data.ASSINAFY_BASE_URL).toBeUndefined();
    }
  });
});
